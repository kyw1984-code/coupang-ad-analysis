    ...
    try:
        ...
        with col3:
            ...
            if total_perf['ROAS'] < 3.0:
                # ROAS가 낮을 땐 목표수익률을 상향하여 효율을 높여야 함
                st.write(f"- **조정**: 수익성이 낮습니다. **목표수익률을 10~20% 상향** 조정하여 효율을 개선하세요.")
            elif total_perf['ROAS'] > 6.0:
                # ROAS가 높을 땐 목표수익률을 하향하여 매출을 극대화해야 함
                st.write(f"- **확장**: 고효율 구간입니다. **목표수익률을 하향** 조정하여 노출을 더 확보하고 매출을 키우세요.")
            else:
                st.write("- **유지**: 현재 수익 구조가 안정적입니다. 현 설정을 유지하세요.")
