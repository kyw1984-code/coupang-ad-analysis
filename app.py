    ...
    try:
        ...
        with col3:
            ...
            if roas_val < 2.0:
                ...
                st.write("- **목표수익률 조정**: **현재 설정값에서 50~100%p 즉시 상향**하세요. (노출을 줄여 누수를 막아야 함)")
                ...
                st.write("- **운영**: 효율이 낮은 검색 지면(기타 지면) 비중을 낮추고 메인 타겟에 집중하세요.")
            elif 4.0 <= roas_val < 6.0:
                st.write("✅ **[안정] 수익 유지 구간**")
                st.write("- **분석**: 수익과 매출 볼륨의 밸런스가 좋습니다.")
                st.write("- **목표수익률 조정**: **현 설정을 유지**하거나, 매출 확대를 위해 10%p씩 미세 하향 조정을 시도하세요.")
                st.write("- **운영**: 경쟁사 동향을 체크하며 점유율을 뺏기지 않도록 관리하세요.")
            else:
                st.write("🚀 **[확장] 고효율 성장 구간**")
                st.write("- **분석**: 수익성이 매우 뛰어납니다. 더 큰 매출을 만들 기회입니다.")
                st.write("- **목표수익률 조정**: **현재 설정값에서 20~50%p 과감히 하향**하세요. (노출을 늘려 볼륨을 키워야 함)")
                st.write("- **운영**: 예산을 증액하고 노출 순위를 상단으로 끌어올려 시장을 선점하세요.")
...
# 푸터
st.divider()
st.markdown("<div style='text-align: center;'><a href='https://hoonpro.liveklass.com/' target='_blank'>🏠 쇼크트리 훈프로 홈페이지 바로가기</a></div>", unsafe_allow_html=True)import streamlit as st
import pandas as pd
